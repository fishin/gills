...
exports.updateUser = function (userId, payload, cb) {
    ...
    Wreck.get(internals.User.settings.api.url + '/user/' + userId, function (err, resp, pl) {
        ...
        Wreck.put(internals.User.settings.api.url + '/user/' + userId, { headers: { 'Content-Type': 'application/json' }, payload: JSON.stringify(config) }, function (err2, resp2, pl2) {

            var updateConfig = JSON.parse(pl2);
