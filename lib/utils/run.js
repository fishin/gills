...
exports.getRunConfig = function (jobId, runId, cb) {
    ...
    internals.Run.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get(internals.Run.settings.api.url + '/job/' + jobId, function (err, resp, pl) {
            ...
            Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId, function (err2, resp2, pl2) {

                var run = JSON.parse(pl2);
                config.runId = run.id;
                config.status = run.status;
                config.commit = run.commit;
                config.checkout = run.checkout;
                config.shortRunId = run.id.split('-')[0];
                Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/runs', function (err3, resp3, pl3) {

                    var runs = JSON.parse(pl3);
                    ...
                    Wreck.post(internals.Run.settings.api.url + '/job/' + jobId + '/commits/compare', { payload: JSON.stringify(comparePayload) }, function (err4, resp4, pl4) {

                        config.compareCommits = JSON.parse(pl4);
                        Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + run.id + '/test/lab.json', function (err5, resp5, pl5) {

                            if (pl5) {
                                config.testResult = JSON.parse(pl5);
                            }
                            Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId + '/archive', function (err6, resp6, pl6) {

                                var files = JSON.parse(pl6);
                                config.archiveFiles = [];
