            ...
            Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId, { json: true }, function (err2, resp2, pl2) {
                ...
                Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/runs', { json: true }, function (err3, resp3, pl3) {
                    ...
                    Wreck.post(internals.Run.settings.api.url + '/job/' + jobId + '/commits/compare', { payload: JSON.stringify(comparePayload), json: true }, function (err4, resp4, pl4) {
                        ...
                        Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId + '/test', { json: true }, function (err5, resp5, pl5) {
                            ...
                            if (pl5 !== null && !pl5.statusCode) {
                                ...
                                if (config.testResult.lint) {
                                    config.lintErrors = 0;
                                    for (var j = 0; j < config.testResult.lint.length; j++) {
                                        config.lintErrors += config.testResult.lint[j].errors.length;
                                    }
                                }
                            ...
                            Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId + '/archive', { json: true }, function (err6, resp6, pl6) {
                                ...
                                if (run.checkout && run.checkout.commands) {
                                    for (var l = 0; l < run.checkout.commands.length; l++) {
                                        //console.log(run.checkout.commands[l]);
                                        var startCheckoutTime = run.checkout.commands[l].startTime;
                                        if (run.checkout.commands[l].startTime) {
                                            run.checkout.commands[l].startTime = DateFormat(startCheckoutTime, 'yyyy-mm-dd HH:MM:ss');
                                        }
                                        if (run.checkout.commands[l].finishTime) {
                                            run.checkout.commands[l].elapsedTime = run.checkout.commands[l].finishTime - startCheckoutTime;
                                            run.checkout.commands[l].finishTime = DateFormat(run.checkout.commands[l].finishTime, 'yyyy-mm-dd HH:MM:ss');
                                        }
                                        config.commands.push(run.checkout.commands[l]);
                                    }
                                }
                                for (var m = 0; m < run.commands.length; m++) {
                                    //console.log(run.commands[m]);
                                    var startTime = run.commands[m].startTime;
                                    if (run.commands[m].startTime) {
                                        run.commands[m].startTime = DateFormat(startTime, 'yyyy-mm-dd HH:MM:ss');
                                    }
                                    if (run.commands[m].finishTime) {
                                        run.commands[m].elapsedTime = run.commands[m].finishTime - startTime;
                                        run.commands[m].finishTime = DateFormat(run.commands[m].finishTime, 'yyyy-mm-dd HH:MM:ss');
                                    }
                                    config.commands.push(run.commands[m]);
                                }
    ...
    internals.Run.settings.Utils.Common.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId, { json: 'true' }, function (err, resp, pl) {
            ...
            Wreck.get(internals.Run.settings.api.url + '/job/' + jobId, { json: 'true' }, function (err2, resp2, pl2) {
                ...
                Wreck.get(internals.Run.settings.api.url + '/job/' + jobId + '/run/' + runId + '/test', { json: 'true' }, function (err3, resp3, pl3) {
...
exports.deleteRuns = function (jobId, cb) {
    ...
    Wreck.delete(internals.Run.settings.api.url + '/job/' + jobId + '/runs', function (err, resp, pl) {
