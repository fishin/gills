...
exports.getTestConfig = function (jobId, runId, artifact) {
    ...
    if (contents) {
        ...
        testResult.totalDuration = tests.duration;
        testResult.totalLeaks = tests.leaks.length;
        testResult.tests = tests.tests;
        for (key in testResult.tests) {
            // add counter for each test
            for (var i = 0; i < tests.tests[key].length; i++) {
                testResult.totalTests++;
                testResult.tests[key][i].testNum = testResult.totalTests;
            }
        }
        //console.log('totalTests: ' + testResult.totalTests);
...
exports.getCoverageConfig = function (jobId, runId, artifact) {
    ...
    if (contents) {
        var tests = JSON.parse(contents);
        testResult.coveragePercent = Math.round(tests.coverage.percent * 100) / 100;
        testResult.coverage = tests.coverage;
    } else {
        testResult = null;
    }
    var config = {
