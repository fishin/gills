...
exports.startJob = function (jobId, cb) {
    ...
    Wreck.get('http://localhost:8081/api/job/' + jobId + '/start', function(err, resp, pl) {
...
exports.startPullRequest = function (jobId, number, token, cb) {
    ...
    //internals.Job.settings.plugins.tacklebox.getPullRequest(jobId, number, token, function(pr) {
    bait.getPullRequest(jobId, number, token, function(pr) {
...
exports.getJobsConfig = function (cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get('http://localhost:8081/api/jobs', function(err, resp, pl) {

            var jobs = JSON.parse(pl);
            var getLastName = function getLastRun(i) {

                if (i < jobs.length) {
                    Wreck.get('http://localhost:8081/api/job/' + jobs[i].id + '/run/byname/last', function(err, resp, pl) {
...
exports.createJob = function (payload, cb) {
    ...
    var bait = new Bait(internals.Job.settings.job);
    var fishhook = new FishHook(internals.Job.settings);
    var headCommands = [];
    var bodyCommands = [];
    var tailCommands = [];
    ...
    Wreck.post('http://localhost:8081/api/job', { payload: JSON.stringify(config) }, function(err, resp, pl) {

        var createConfig = JSON.parse(pl);
        var jobId = createConfig.id;
        if (config.schedule.type === 'cron') {
            //internals.Job.settings.plugins.tacklebox.startSchedule(createConfig);
            fishhook.startSchedule(createConfig);
        }
...
exports.getJobConfig = function (jobId, token, cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        var finishedRuns = [];
        var activeRuns = [];
        Wreck.get('http://localhost:8081/api/job/' + jobId, function(err, resp, pl) {
            ...
            Wreck.get('http://localhost:8081/api/job/' + jobId + '/runs', function(err, resp, pl) {
                ...
                if (runs.length > 0) {
                    ...
                    if (runs.length > 1) {
                        if (runs[1].commit) {
                            ...
                            config.compareCommits = compareCommits;
                        }
                    }
                }
                ...
                var pids = [];
                Wreck.get('http://localhost:8081/api/job/' + jobId + '/run/' + lastRunId + '/pids', function(err, resp, pl) {
                    ...
                    Wreck.get('http://localhost:8081/api/job/' + jobId + '/run/' + lastRunId + '/test/lab.json', function(err, resp, pl) {
...
exports.updateJob = function (jobId, payload, cb) {
    ...
    //console.log(jobId);
    Wreck.post('http://localhost:8081/api/job/' + jobId, { payload: JSON.stringify(config) }, function(err, resp, pl) {

        var updateConfig = JSON.parse(pl);
        //internals.Job.settings.plugins.tacklebox.stopSchedule(jobId);
        fishhook.stopSchedule(jobId);
        if (config.schedule.type === 'cron') {
            //internals.Job.settings.plugins.tacklebox.startSchedule(updateConfig);
            fishhook.startSchedule(updateConfig);
        }
        return cb();
    });
};
...
exports.deleteJob = function (jobId, cb) {
    ...
    //internals.Job.settings.plugins.tacklebox.stopSchedule(jobId);
    fishhook.stopSchedule(jobId);
    Wreck.delete('http://localhost:8081/api/job/' + jobId, function(err, resp, pl) {
...
exports.deleteWorkspace = function (jobId, cb) {
    ...
    Wreck.delete('http://localhost:8081/api/job/' + jobId + '/workspace', function(err, resp, pl) {
...
exports.getCommitsConfig = function(jobId, cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get('http://localhost:8081/api/job/' + jobId, function (err, resp, pl) {
            ...
            Wreck.get('http://localhost:8081/api/job/' + jobId + '/commits', function (err, resp, pl) {
...
exports.retryPullRequest = function(jobId, number) {

    var bait = new Bait(internals.Job.settings.job);
    console.log('retrying pull request: ' + number);
    //internals.Job.settings.plugins.tacklebox.deletePullRequest(jobId, number);
...
//    return cb(config);
    var bait = new Bait(internals.Job.settings.job);
    Wreck.get('http://localhost:8081/api/job/' + jobId + '/prs', function (err, resp, pl) {
