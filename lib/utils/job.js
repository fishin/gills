    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get(internals.Job.settings.api.url + '/jobs', function (err, resp, pl) {
            ...
            var getLastName = function getLastRun (i) {
                ...
                if (i < jobs.length) {
                    Wreck.get(internals.Job.settings.api.url + '/job/' + jobs[i].id + '/run/byname/last', { json: 'force' }, function (err2, resp2, pl2) {
...
exports.getJobCreateConfig = function (cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        var config = {
            ...
            notifyMessage: internals.Job.settings.server.info.uri + internals.Job.settings.viewPath + '/{relativeUrl}',
...
exports.getJobConfig = function (jobId, cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get(internals.Job.settings.api.url + '/job/' + jobId, { json: 'force' }, function (err, resp, pl) {
            ...
            if (job.body) {
                for (var k = 0; k < job.body.length; k++) {
                    var keyName2 = 'bodyCommand' + k;
                    config[keyName2] = job.body[k];
                }
            }
            if (job.tail) {
            ...
            Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/runs', function (err2, resp2, pl2) {

                var runs = JSON.parse(pl2);
                ...
                Wreck.post(internals.Job.settings.api.url + '/job/' + jobId + '/commits/compare', { payload: JSON.stringify(comparePayload) }, function (err3, resp3, pl3) {

                    config.compareCommits = JSON.parse(pl3);
                    Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/run/' + lastRunId + '/pids', function (err4, resp4, pl4) {

                        var pids = JSON.parse(pl4);
                        if (pids[0]) {
                            // remove the finished Runs first one as it was really active
                            finishedRuns.splice(0, 1);
                        }
                        config.finishedRuns = finishedRuns;
                        Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/run/' + lastRunId + '/test/lab.json', { json: 'force' }, function (err5, resp5, pl5) {
...
exports.getCommitsConfig = function (jobId, cb) {
    ...
    internals.Job.getCommonConfig(function (commonConfig) {
        ...
        Wreck.get(internals.Job.settings.api.url + '/job/' + jobId, function (err, resp, pl) {
            ...
            Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/commits', function (err2, resp2, pl2) {

                //console.log(pl2);
                var commits = JSON.parse(pl2);
                // add repoUrl
