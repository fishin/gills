...
exports.getJobConfig = function (jobId) {
    ...
    if (runs.length > 0) {
        lastRunId = runs[0].id;
        //testResults = internals.getTestResult(jobId, lastRunId, 'lab.json');
        testResults = internals.Job.settings.plugins.tacklebox.getTestResult(jobId, lastRunId, 'lab.json');
        var commits = internals.getCommits(jobId);
        //var commits = internals.Job.settings.plugins.tacklebox.getCommits(jobId);
        if (commits.length > 0) {
...
/*
internals.getTestResult = function (jobId, runId, artifact) {

    var hatchery = new Hatchery({ dirPath: internals.Job.settings.job.dirPath + '/' + jobId + '/' + runId });
...
internals.getCommits = function(jobId) {
   ...
   var lastRun = internals.Job.settings.plugins.tacklebox.getRunByName(jobId, 'last');
   var job = internals.Job.settings.plugins.tacklebox.getJob(jobId);
   var gitLog = [];
   var gitLogBuffer = '';
   ...
   if (lastRun) {
       for (var i = 0; i < lastRun.commands.length; i++) {
           if (lastRun.commands[i].command.match('git log')) {
