...
exports.startJob = function (jobId) {
    ...
    var runId = internals.Job.settings.plugins.tacklebox.startJob(jobId, null);
    return runId;
};
...
exports.createJob = function (pl) {
    ...
    if (config.schedule.type === 'cron') {
        internals.Job.settings.plugins.tacklebox.startSchedule(jobId);
    }
    return createConfig;
};
...
exports.getJobConfig = function (jobId, token, cb) {
    ...
    var runs = internals.Job.settings.plugins.tacklebox.getRuns(jobId, null);
    var lastRunId = null;
    var lastCommit;
    var compareCommits;
    var testResult = [];
    ...
    for (var i = 0 ; i < runs.length; i++) {
        ...
        var runId = runs[i].id;
        var run = internals.Job.settings.plugins.tacklebox.getRun(jobId, null, runId);
        ...
        var pids = []; 
        if (i === 0) {
            pids = internals.Job.settings.plugins.tacklebox.getRunPids(jobId, null, runId); 
        }
        if (pids[0]) {
            //console.log(pids);
            // figure out prev run
...
internals.getPullRequestsConfig = function(jobId, token, lastRunId, config, cb) {

    internals.Job.settings.plugins.tacklebox.getPullRequests(jobId, token, function(prs) {

        for (var i = 0; i < prs.length; i++) {
            var totalChecked = 0;
            var totalNew = 0;
            prs[i].jobId = jobId;
            // figure out if this pr is run
            var prRuns = internals.Job.settings.plugins.tacklebox.getRuns(jobId, prs[i].number);
            if (prRuns.length === 0) {
                totalNew++;
                console.log('starting pr check for: ' + prs[i].number);
                if (internals.Job.settings.prs.autoStart) {
                    internals.Job.settings.plugins.tacklebox.startJob(jobId, prs[i].number);
                }
            } else {
                var prRun = internals.Job.settings.plugins.tacklebox.getRun(jobId, prs[i].number, prRuns[0].id);
                ...
                var runId = prRun.id;
                var pids = internals.Job.settings.plugins.tacklebox.getRunPids(jobId, prs[i].number, runId);
                ...
                if (pids.length > 0) {
                    ...
                    config.activePullRequests.push({ percent: percent, elapsedTime: elapsedTime, processes: processes });
                }
                prs[i].checked = {};
                if (prRun.status === 'succeeded' ) {
                    prs[i].checked.classColor = 'success';
                    prs[i].checked.buttonColor = 'btn-success';
                }
                if (prRun.status === 'failed') {
                    prs[i].checked.classColor = 'danger';
                    prs[i].checked.buttonColor = 'btn-danger';
                }
                totalChecked++;
            }
        }
        config.prs = prs;
        config.totalChecked = totalChecked;
        config.totalNew = totalNew;
        return cb(config);
    });
};
