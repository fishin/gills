...
exports.getJobsConfig = function (cb) {
    ...
    Wreck.get(internals.Job.settings.api.url + '/jobs', { json: true }, function (err, resp, pl) {
        ...
        var jobs = pl;
        internals.getFullJobs(0, jobs, [], function (fullJobs) {
...
internals.getFullJobs = function (i, jobs, fullJobs, glrcb) {
        ...
        Wreck.get(internals.Job.settings.api.url + '/job/' + jobs[i].id + '/run/byname/last', { json: true }, function (err2, resp2, pl2) {
            ...
            if (pl2 !== null && !pl2.statusCode) {
                ...
                if (lastRun.status === 'cancelled') {
                    job.classColor = 'warning';
                }
            }
            Wreck.get(internals.Job.settings.api.url + '/job/' + job.jobId + '/run/' + job.runId + '/test', { json: true }, function (err3, resp3, pl3) {
                ...
                fullJobs.push(job);
                return internals.getFullJobs(i + 1, jobs, fullJobs, glrcb);
            });
        });
    } else {
        return glrcb(fullJobs);
    }
};
