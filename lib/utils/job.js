    ...
    Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/prs', { json: 'force' }, function (err, resp, pl) {
        ...
        var getLastName = function getLastRun (i) {
            ...
            if (i < prs.length) {
                ...
                Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/pr/' + prs[i].number + '/runs', { json: 'force' }, function (err, resp, pl) {

                    var prRuns = [];
                    if (prRuns !== null && !pl.statusCode) {
                        prRuns = pl;
                    }
                    if (prRuns.length === 0) {
                        // its new
                        ...
                        Wreck.get(internals.Job.settings.api.url + '/job/' + jobId + '/pr/' + prs[i].number + '/run/' + runId + '/pids', { json: 'force' }, function (err, resp, pl) {
