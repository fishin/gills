    ...
    Wreck.get(internals.Common.settings.api.url + '/reels', { json: 'true' }, function (err, resp, pl) {
        ...
        Wreck.get(internals.Common.settings.api.url + '/queue', { json: 'true' }, function (err2, resp2, pl2) {
            ...
            Wreck.get(internals.Common.settings.api.url + '/jobs/active', { json: 'true' }, function (err3, resp3, pl3) {
                ...
                var iterateActiveJobs = function (j) {

                    if (j < jobsKeys.length) {
                        var jobId = jobsKeys[j];
                        ...
                        Wreck.get(internals.Common.settings.api.url + '/job/' + jobId + '/runs', { json: 'force' }, function (err4, resp4, pl4) {
                            ...
                            for (var k = 0; k < runs.length; k++) {
                                if (runs[k].id === activeJobs[jobId].runId) {
                                    run = runs[k];
                                    runIndex = k;
                                }
                            }
                            ...
                            for (var l = 0; l < activeJobs[jobId].pids.length; l++) {
                                var process = {
                                    jobId: jobId,
                                    runId: activeJobs[jobId].runId,
                                    pid: activeJobs[jobId].pids[l],
                                    command: command
                                };
                                activeJobs[jobId].processes.push(process);
                            }
                            iterateActiveJobs(j + 1);
                        });
                    }
                };
                iterateActiveJobs(0);
                ...
                Wreck.get(internals.Common.settings.api.url + '/prs/active', { json: 'true' }, function (err5, resp5, pl5) {
                    ...
                    var iterateActiveJobs2 = function (m, jobsArray, jobscb) {

                        if (m < pullRequestsJobsKeys.length) {
                            var jobId = pullRequestsJobsKeys[m];
                            ...
                            internals.iterateActivePullRequests(0, jobId, prs, [], function (prArray) {
                                ...
                                return iterateActiveJobs2(m + 1, jobsArray, jobscb);
                            });
                        } else {
                    ...
                    iterateActiveJobs2(0, [], function (pullRequestsArray) {
                        ...
                        var config = {
                            reels: reels,
                            queue: queue,
                            //load: internals.Common.settings.server.load,
