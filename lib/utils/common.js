...
internals.getFullActiveJobs = function (i, fullActiveJobs, iajcb) {
    ...
    if (i < jobsKeys.length) {
        ...
        Wreck.get(internals.Common.settings.api.url + '/job/' + jobId + '/runs', { json: 'force' }, function (err, resp, pl) {
            ...
            var command = internals.getActiveCommand(run.commands);
            fullActiveJobs[jobId].processes = [];
            for (var k = 0; k < fullActiveJobs[jobId].pids.length; k++) {
                var process = {
                    jobId: jobId,
                    runId: fullActiveJobs[jobId].runId,
                    pid: fullActiveJobs[jobId].pids[k],
                    command: command
                };
                fullActiveJobs[jobId].processes.push(process);
            }
