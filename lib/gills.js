...
exports.getQueue = function (request, reply) {
    ...
    var config = {};
    if (request.params.id) {
        queue = internals.Gills.settings.plugins.tacklebox.getQueue(request.params.id);
        config = {
            post_url: '/gills/queue/'+queue.id,
            id: queue.id,
            name: queue.name,
        ...
        config.post_url = '/gills/queue';
    }
    config.view_queue = true; 
    config.angler = 'lloyd';
    reply.view('layout', config);   
};
...
exports.createQueue = function (request, reply) {
    ...
    //console.log(config);
    //var createConfig = internals.Gills.settings.plugins.tacklebox.createQueue(config);
