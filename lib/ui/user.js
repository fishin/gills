        ...
        if (request.payload.name === 'admin') {
            Wreck.get(internals.User.settings.api.url + '/user/byname/' + request.payload.name, function (err, resp, pl) {
                ...
                if (!adminUser) {
                    ...
                    Wreck.post(internals.User.settings.api.url + '/user/' + adminUser.id + '/validate', { headers: { 'Content-Type': 'application/json' }, payload: JSON.stringify(config) }, function (err, resp, pl) {
