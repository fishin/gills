...
exports.loginUser = function (request, reply) {
    ...
    if (request.payload.name) {
        if (request.payload.name === 'admin') {
            Wreck.get(internals.User.settings.api.url + '/user/byname/' + request.payload.name, { json: true }, function (err, resp, pl) {
                ...
                if (!adminUser) {
                    ...
                    Wreck.post(internals.User.settings.api.url + '/user', { headers: { 'Content-Type': 'application/json' }, payload: JSON.stringify(adminConfig), json: true }, function (err2, resp2, pl2) {
                    ...
                    Wreck.post(internals.User.settings.api.url + '/user/' + adminUser.id + '/validate', { headers: { 'Content-Type': 'application/json' }, payload: JSON.stringify(userConfig), json: true }, function (err3, resp3, pl3) {
            ...
            // normal user
            Wreck.get(internals.User.settings.api.url + '/user/byname/' + request.payload.name, { json: true }, function (err4, resp4, pl4) {
                ...
                if (pl4) {
                    ...
                    Wreck.post(internals.User.settings.api.url + '/user/' + user.id + '/validate', { headers: { 'Content-Type': 'application/json' }, payload: JSON.stringify(config), json: true }, function (err5, resp5, pl5) {
...
exports.loginGithub = function (request, reply) {
    ...
    if (request.auth.isAuthenticated) {
        Wreck.get(internals.User.settings.api.url + '/user/byname/' + request.auth.credentials.profile.username, { json: true }, function (err, resp, pl) {
